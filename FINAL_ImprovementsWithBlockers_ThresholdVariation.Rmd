...
df<- as.data.frame(df)
df$Mean<- rep(c(rep("Sample",4),"Mean"),7)

#plot
p<- ggplot(df, aes(x=threshold, y=BlockerMinusNoBlocker, color=ID))+
  geom_line(aes(linetype = Mean), size=1.5, show.legend=FALSE)+
  geom_point(size=2)+
  scale_y_continuous(breaks=seq(0, 1000, 50))+
  scale_color_manual(values=c("turquoise2", "blue4", "dodgerblue", "lightskyblue","black"))+
  ggtitle("Number of miRNAs passing a count threshold")+
  theme_classic()
p

```
*Might want to think of an alternative way of visualizing this

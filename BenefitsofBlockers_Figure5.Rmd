...
thresholds<-c(0,5,10,25,50,75,100,250,500)
...
#Potential Supplemental Table 2: all miRs DE in either analysis + whether they were DE in both or one + pvalue + log2 FC + Raw counts for that species
```{r}
...
#Plot
p<- ggplot(All_df, aes(x=Sig, y=median, fill=Group, group=Order))+
...
```
<<<<<<< Updated upstream:BenefitsofBlockers_Figure5.Rmd
...
```
=======
>>>>>>> Stashed changes:FINAL_032520_Figure5_BenefitsofBlockers.Rmd

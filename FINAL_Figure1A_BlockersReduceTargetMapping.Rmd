...
```{r}
counts<- downsample.counts(countsInput, set.seed(42)) #9,989,167 lowest B.570.0 / 7,970,014 in BaseSpace  (C-blocked)
```
...
MapAnyTarget<- counts[rownames(counts) %in% c("hsa-miR-92a", "hsa-miR-451a", "hsa-miR-486"),] #for basespace collapsed
...
#p-value = 1.524e-09 / BS: 7.8 e-9
```



#3. Figure 1B: Graph % for any target for all 12 samples (A and B, all replicates), grouping by patient then by blocked/unblocked then by ordering input levels (input labels added manually). Optionally, also include samples C and D 
```{r}
#Only A and B
...
#Alternative: plotting all samples on same graph, coloring based on sample
...
c_MapAnyTarget<- c_counts[rownames(c_counts) %in% c("hsa-miR-92a", "hsa-miR-451a", "hsa-miR-486"),] #For BaseSpace collapsed
...
#PM_melt$Order<- 1:24 #for command line or Basespace collapsed
PM_melt$Order<- 1:nrow(c_MapAnyTarget) #For Basespace not collapsed

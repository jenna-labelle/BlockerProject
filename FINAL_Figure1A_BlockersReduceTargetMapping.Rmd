...
p<-ggplot(Per_AnyTarget_TwoGroups, aes(y=MeanPercentMapping, x=reorder(Status, - MeanPercentMapping),fill=Status))+
  ...
  geom_errorbar(aes(ymin=MeanPercentMapping-sd, ymax=MeanPercentMapping+sd), width=.1, position=position_dodge(.9),color="black")+
  ylim(0,100)+
...
#3. Graph % for any target for all 12 samples (A and B, all replicates), grouping by patient then by blocked/unblocked then by ordering input levels (input labels added manually)
```{r}
...
Per_AnyTarget_16Samples<- data.frame(Status=c(rep("Unblocked",6),rep( "Blocked",6)), 
                                     ...
                                     Patient=substr(names(JustReplicated), 1,1),
...
#plot
p<-ggplot(PM_melt, aes(y=value, x=ID, color=Status, fill=Status))+
  ...
  ylab("Percent of reads mapping to target species")+
  xlab("")+
  scale_fill_manual("", values=c("A:UnBlocked"="turquoise3","Blocked"="firebrick3"))+
  ylim(0,100)

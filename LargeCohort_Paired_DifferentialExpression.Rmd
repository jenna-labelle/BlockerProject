...
blocked<-t(counts_OutliersRemoved[rownames(counts_OutliersRemoved)=="hsa-miR-486",grep("Blocked", colnames(counts_OutliersRemoved))])
unblocked<-t(counts_OutliersRemoved[rownames(counts_OutliersRemoved)=="hsa-miR-486",grep("Unblocked", colnames(counts_OutliersRemoved))])
...
blocked_pheno$Reduction<-(unblocked_pheno$x - blocked_pheno$x)/unblocked_pheno$x

#Combine for plotting
df<-data.frame(Abundance=unblocked_pheno$x, ReducedBy=blocked_pheno$Reduction)

#Regression
fit <- lm(Abundance ~ ReducedBy, data = df)

#Plot
p<-ggplot(df, aes(x=Abundance, y=ReducedBy))+

...
#Plot- mapping to any target,combined- 530 and 535 total samples
```{r}
...
#Plot- mapping to all targets separately- 530 and 535 total samples
```{r}
...
#export
write.csv(Ttest, paste(writewd, "Graphs_Blocker/FinalFigures/TtestPValues_LargeCohort_PerMappingForAllmiRs.csv",sep=""))
```
```{r}
...
#Export
#write.csv(counts, paste(writewd, "LargeCohort_AllSamples_RawCounts_CPMFilter.csv", sep=""))
...
#write.csv(meta, paste(writewd,"LargeCohort_Metadata.csv",sep=""))
```
...
sig<- sig[abs(sig$log2FoldChange)>1,] #177 92
sig<-sig[sig$baseMean>=50,] #71- optional
...
#Plot the number of species passing count thresholds for blocked and unblocked datasets
...
```{r}
#Get raw (downsampled) counts for blocked and unblocked datasets
...
```{r}
#Write off targets to CSV
write.csv(OT, paste(wd, "LargeCohort_OffTargets.csv",sep=""))
```

...
TargetCPM<- as.data.frame(do.call(rbind, TargetCPM_1))
...
#Export
write.csv(counts, paste(readwd, "LargeCohort_AllSamples_RawCounts_CPMFilter.csv", sep=""))
...
nrow(resOrdered[resOrdered$padj<0.01,]) #312
nrow(resOrdered[resOrdered$padj<0.05,]) #319
sig<-resOrdered[resOrdered$padj<0.01,]
sig<- sig[abs(sig$log2FoldChange)>1,] #177

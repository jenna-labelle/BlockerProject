...
#3: Read in filtered/merged unblocked data from step 1 --> Collapse technical replicates --> Write out for later use
```{r}
...
#4:  Read in filtered/merged blocked data from step 1 --> Collapse technical replicates --> Write out for later use
```{r}
...
#5: Optional: get only samples that are in a blocked/unblocked pair (from same subject)
...
unblocked_Counts<- read.csv(paste(wd, "UnblockedRawCounts_TechCollapsed.csv",sep=""))
...
blocked_Counts<- read.csv(paste(wd, "BlockedRawCounts_TechCollapsed.csv",sep=""))
colnames(blocked_Counts)<-paste(colnames(blocked_Counts), "Blocked", sep="_")

#Subset raw counts to paired samples -89 total pairs
paired_unblocked<-unblocked_Counts[,colnames(unblocked_Counts) %in% pairedSamples$NewSampleID]
...
#write to csv- can use this or the unpaired data set for the rest of this analysis
write.csv(paired_unblocked, paste(wd, "LargeCohort_PairedSubset_89Samples_Unblocked.csv", sep=""))
write.csv(paired_blocked, paste(wd, "LargeCohort_PairedSubset_89Samples_Blocked.csv", sep=""))
```
...
#10: Differential expression for paired cohort samples. Pair-wise DESeq2 used. Metadata includes sample information from BH.
```{r}
...
CPM_Above1<- CPM[apply(CPM, 1, function(x) {sum(x>=1)})>(ncol(CPM)*.50),]
...
#create DESeq2 object- just based on samplegroup (blockerstatus)
#will add samples to model later (i.e., pairwise analysis)
dds<- DESeqDataSetFromMatrix(countData=counts_collapsed_paired, 
...
nrow(resOrdered_paired[resOrdered_paired$padj<0.05,]) #270 (385 if unpaired)
nrow(resOrdered_paired[resOrdered_paired$padj<0.01,]) #244 (295 if unpaired)
nrow(resOrdered_paired[resOrdered_paired$padj<0.01 & abs(resOrdered_paired$log2FoldChange)>1,]) #138 (206 if unpaired)
...
```{r}
writewd<-"C:/Users/Jenna/Documents/BlockerProject/"
blocked_counts<-read.csv( paste(writewd, "BlockedRawCounts_TechCollapsed.csv", sep=""), row.names = "X")
unblocked_counts<-read.csv( paste(writewd, "UnblockedRawCounts_TechCollapsed.csv", sep=""), row.names = "X")
...
#Get counts passing threshold for unblocked samples, then transform data- 4 columns: ID, threshold, #of species that passed that threshold, and blocker status
        ...
        unblockedPassThreshold$status<- rep("unblocked", length(thresholds) * (518+1))
...
#Get counts passing threshold for blocked samples, then transform data- 4 columns: ID, threshold, #of species that passed that threshold, and blocker status
        ...
        blockedPassThreshold$status<- rep("blocked", length(thresholds) * (525+1))

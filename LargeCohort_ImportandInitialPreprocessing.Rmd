...
for (i in 1:length(unblocked_input)){
        sampleIDs<-paste(colnames(unblocked_input[[1]]), "_Unblocked",sep="")
...
for (i in 1:length(blocked_input)){
        sampleIDs<-paste(colnames(blocked_input[[1]]), "_Blocked",sep="")
        samples=c(samples, sampleIDs)
        batches=c(batches, rep(paste("Batch_B", i,sep=""),length(sampleIDs)))}
        
b_batches<-data.frame(samples, batches)

#Merge with unblocked batches, remove "X" samples and remove "trimmed"
batches<-rbind(ub_batches, b_batches)
batches<-batches[!(grepl("X", batches$samples)),]
batches$samples<- gsub(paste0(c(".x", ".y", "Trimmed.x", "Trimmed.y", "Trimmed", 
                                     "trimmed.x", "trimmed.y", "trimmed"), collapse="|"),"", batches$samples)

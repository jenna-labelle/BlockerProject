...
colnames(OT)<- c("Species", "baseMean", "log2FoldChange", "padj", "MeanRawUnblockedCounts")
OT<- merge(OT, Blocked_RawCounts, by=0)
OT<-OT[,-1]
colnames(OT)<- c("Species", "baseMean", "log2FoldChange", "padj", "MeanRawUnblockedCounts", "MeanRawBlockedCounts")
...
```{r}
write.csv(OT, paste(wd,"OffTargetStats_042020.csv", sep=""))
```
...
#legend
dummy<-data.frame(MeanRawCounts=c(10,100,1000,10000,100000,300000), Species=c("a", "b", "c", "d", "e", "f"),
                  padj=seq(1,6,1), log2FoldChange=seq(1,6,1))


#plot
p<-ggplot(dummy,aes(x=log2FoldChange, y= padj))+
  geom_point(aes(size=MeanRawCounts), pch=21, color="black")+
